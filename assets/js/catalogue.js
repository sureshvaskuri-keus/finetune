(() => {
  ...
  const catalogueTextCache=new Map();
  ...
  async function fetchCatalogueText(file){
    if(catalogueTextCache.has(file))return catalogueTextCache.get(file);
    const request=fetch(encodeURI(file),{cache:'default'}).then(response=>{
      if(!response.ok)throw new Error(`${response.status} ${response.statusText}`);
      return response.text();
    }).catch(error=>{
      catalogueTextCache.delete(file);
      throw error;
    });
    catalogueTextCache.set(file,request);
    return request;
  }
  ...
  function searchMatches(r,q){return !q||r.searchText.includes(q)}
  ...
  function initKeusFilterSelects(){
    document.querySelectorAll('select.field').forEach(select=>{
      ...
      trigger.addEventListener('keydown',e=>{
        ...
        closeAllKeusFilterMenus();
        closeKeusProductMenu();
        openKeusFilterMenu(wrap,e.key==='ArrowUp'||e.key==='End'?'last':'active');
      });
      menu.addEventListener('keydown',e=>handleListboxKeydown(e,menu,trigger,()=>closeKeusFilterMenu(wrap)));
    });
    syncKeusFilterMenus();
  }
  ...
  function syncKeusFilterMenu(wrap){
    ...
    menu.querySelectorAll('[data-filter-value]').forEach(btn=>btn.addEventListener('click',e=>{
      ...
      triggerFocus(wrap.querySelector('.keus-filter-trigger'));
    }));
  }
  ...
  function triggerFocus(element){if(element)requestAnimationFrame(()=>element.focus())}

  function handleListboxKeydown(event,menu,trigger,close){
    ...
    else if(event.key==='Escape'){event.preventDefault();close();triggerFocus(trigger);return}
  ...
  function openKeusFilterMenu(wrap,focusTarget='active'){
    const menu=wrap.querySelector('.keus-filter-menu');
    ...
    const options=[...menu.querySelectorAll('.keus-filter-option')];
    const active=menu.querySelector('.keus-filter-option.active')||options[0];
    const target=focusTarget==='last'?options.at(-1):active;
    if(target)setTimeout(()=>{target.focus();target.scrollIntoView({block:'nearest'})},0);
  }
  ...
  function openKeusProductMenu(focusTarget='active'){
    ...
    const options=[...menu.querySelectorAll('.keus-product-option')];
    const active=menu.querySelector('.keus-product-option.active')||options[0];
    const target=focusTarget==='last'?options.at(-1):active;
    if(target)setTimeout(()=>{target.focus();target.scrollIntoView({block:'nearest'})},0);
  }
  ...
  async function getCatalogueForSearch(key){
    ...
    const cfg=CATEGORIES[key];
    const text=await fetchCatalogueText(cfg.file);
      ...
      card.innerHTML=`
        ...
        <button class="card-open" type="button">
          <div class="card-image">
            ${image?`<img src="${escapeHtml(optimizeImageUrl(image,420))}" alt="${escapeHtml(title)}" loading="lazy" decoding="async" fetchpriority="low">`:'<span style="font-size:12px;color:#625d56">No image</span>'}
          </div>
          <div class="card-body">
  ...
  function openDetail(group,preferredFinish=''){
    ...
    els.specGrid.innerHTML=[['Vf',summarize(rows.map(v=>v.vf))],['Imax',summarize(rows.map(v=>v.imax))],['Wattage',summarize(rows.map(v=>v.wattage))],['CCT',summarizeCCT(rows.map(v=>v.cct))],['CRI',summarize(rows.map(v=>v.cri))],['Beam Angle',summarizeAll(rows.map(v=>v.beam))],['Cutout',summarize(rows.map(v=>v.cutout))],['Batch Code',summarize(rows.map(v=>v.batchCode))],['Stock Codes',`${unique(rows.map(v=>v.stockCode)).length} available`],['BIS No',summarize(rows.map(v=>v.bisNo))],['BIS SEC',summarizeAll(rows.map(v=>v.bisSpec),' · ')],['MRP',summarize(rows.map(v=>v.mrp))],['MFG BY',summarize(rows.map(v=>v.mfgBy))]].map(([l,v])=>specCell(l,v)).join('');renderDetailFinishOptions(rows);renderDetailImageAndRows(rows);els.modal.hidden=false;els.modal.classList.add('open');document.body.style.overflow='hidden';document.querySelector('main')?.setAttribute('inert','');document.querySelector('header')?.setAttribute('inert','');requestAnimationFrame(()=>els.closeBtn.focus())
  }
  ...
  function closeDetail(){els.modal.classList.remove('open');els.modal.hidden=true;document.body.style.overflow='';document.querySelector('main')?.removeAttribute('inert');document.querySelector('header')?.removeAttribute('inert');activeGroup=null;activeFinish='';activeImage='';triggerFocus(lastFocusedElement);lastFocusedElement=null}

  function trapModalFocus(event){
    if(event.key!=='Tab'||els.modal.hidden)return;
    const focusable=[...els.modal.querySelectorAll('button:not([disabled]),a[href],input:not([disabled]),select:not([disabled]),[tabindex]:not([tabindex="-1"])')].filter(node=>node.offsetParent!==null);
  ...
  async function loadCategory(key){
    ...
    els.categoryTabs.querySelectorAll('[data-category]').forEach(btn=>{const active=btn.dataset.category===key;btn.classList.toggle('active',active);if(active)btn.setAttribute('aria-current','page');else btn.removeAttribute('aria-current')});
    ...
    renderCatalogue();
    try{
      if(globalCatalogueCache[key]){
        records=[...globalCatalogueCache[key].records];
        groups=[...globalCatalogueCache[key].groups];
        populateFilters();
        updateAll();
        els.status.innerHTML=categoryStatusHtml(key);
      }else{
        const text=await fetchCatalogueText(cfg.file);
  ...
  for(const f of filterFields)filterEls[f].addEventListener('change',()=>{syncStateFromUI();currentPage=0;updateAll()});els.moreBtn.addEventListener('click',()=>{const open=els.advancedFilters.classList.toggle('open');els.moreBtn.textContent=open?'Fewer Filters':'More Filters';els.moreBtn.setAttribute('aria-expanded',String(open))});els.resetBtn.addEventListener('click',resetAll);
  ...
  });els.closeBtn.addEventListener('click',closeDetail);els.modal.addEventListener('click',e=>{if(e.target===els.modal)closeDetail()});document.addEventListener('keydown',e=>{if(e.key==='Escape'&&els.modal.classList.contains('open'))closeDetail();trapModalFocus(e)});
  window.addEventListener('resize',()=>{
    clearTimeout(resizeTimer);
    resizeTimer=setTimeout(()=>{currentPage=0;if(document.body.classList.contains('global-search-mode'))renderGlobalSearchResults(state.q);else renderCatalogue()},180);
  });
  ...
  if(productTrigger)productTrigger.addEventListener('keydown',e=>{
    if(!['ArrowDown','ArrowUp','Home','End'].includes(e.key))return;
    e.preventDefault();
    closeAllKeusFilterMenus();
    openKeusProductMenu(e.key==='ArrowUp'||e.key==='End'?'last':'active');
  });
  const productMenu=$('keusProductMenu');
  if(productMenu)productMenu.addEventListener('keydown',e=>handleListboxKeydown(e,productMenu,productTrigger,closeKeusProductMenu));
